...
func  TestFromFloat(t *testing.T){
	...
	for _, tt := range tests {
		dec,_ := NewDecFromFloatForTest(tt.f)
		str := dec.ToString()
...
func TestToHashKey(t *testing.T) {
	...
	for _, ca := range tests {
		...
		for _, num := range ca.numbers {
			var dec MyDecimal
			assert.Nil(t,dec.FromString([]byte(num)))
...
func TestRemoveTrailingZeros(t *testing.T) {
	...
	for _, ca := range tests {
		var dec MyDecimal
		assert.Nil(t,dec.FromString([]byte(ca)))
...
func TestFromString(t *testing.T) {
	...
	for _, ca := range tests {
		var dec MyDecimal
		err := dec.FromString([]byte(ca.input))
		assert.Equal(t,err,ca.err,ca.input)
		result := dec.ToString()
		assert.Equal(t,string(result),ca.output)
	}
	wordBufLen = 1
...
func TestToBinFromBin(t *testing.T) {
	...
	for _, tt := range errTests {
		...
		assert.Equal(t,ErrBadNumber,err)
	}
}
...
func TestReset(t *testing.T) {
	...
	assert.Nil(t,x1.FromString([]byte("38520.130741106671")))
	assert.Nil(t,y1.FromString([]byte("9863.944799797851")))
	assert.Nil(t,DecimalAdd(&x1, &y1, &z1))

	var x2, y2, z2 MyDecimal

	assert.Nil(t,x2.FromString([]byte("121519.080207244")))
	assert.Nil(t,y2.FromString([]byte("54982.444519146")))
	assert.Nil(t,DecimalAdd(&x2, &y2, &z2))
	assert.Nil(t,DecimalAdd(&x2, &y2, &z1))
